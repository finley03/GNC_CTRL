...
void serial_rx_init_dma() {
	...
	// set source address
	dma_descriptor[SERIAL_RX_CHANNEL].SRCADDR = (uint32_t) &(SERCOM2->USART.DATA.reg);
	...
	DMAC->CHCTRLB.reg = DMAC_CHCTRLB_TRIGACT_BEAT | DMAC_CHCTRLB_TRIGSRC(SERCOM2_DMAC_ID_RX) | DMAC_CHCTRLB_LVL_LVL2;
